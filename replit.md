...
- **图片生成**: 聚光Chat API (https://ai.juguang.chat)
  - gemini-2.5-flash-image-preview 模型
  - Base64编码返回，自动转换为Data URL格式
  - 使用 JUGUANG_API_KEY 环境变量
- **提示词优化**: 图片生成自动添加画面比例和质量提示
- 移除对 GEMINI_API_KEY、OPENAI_API_KEY、THIRD_PARTY_API_KEY 和 VOLCENGINE 相关环境变量的依赖
